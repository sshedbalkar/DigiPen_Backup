...
namespace GLEED2D
{
    public partial class CircleItem
    {
        ...
        [XmlIgnore()]
        public float pRadius { get { return Radius; } set { OnTransformed();  Radius = value; } }
        ...
        public CircleItem(Vector2 startpos, float radius)
            : base()
        {
            this.PositionOrig = startpos;
            this.Radius = radius;
            this.FillColor = Constants.Instance.ColorPrimitives;
            this.Scale = new CustomNode("string");
            OnTransformed();
        }
        ...
        {
            return (worldpos - PositionOrig).Length() <= Radius;
        }


        public override void OnTransformed()
        {
            Scale.value = (Radius * 2 / scaleRatio).ToString() + "," + (Radius*2 / scaleRatio).ToString() + ",1";
        }
        ...
        public override void drawInEditor(SpriteBatch sb)
        {
            ...
            Primitives.Instance.drawCircleFilled(sb, PositionOrig, Radius, c);
        }
        ...
        public override void drawSelectionFrame(SpriteBatch sb, Matrix matrix, Color color)
        {
            ...
            Vector2 transformedPosition = Vector2.Transform(PositionOrig, matrix);
