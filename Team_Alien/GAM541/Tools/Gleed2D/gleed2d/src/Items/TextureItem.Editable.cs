...
namespace GLEED2D
{
    ...
    public partial class TextureItem
    {
            ...
            : base()
        {
            ...
            this.PositionOrig -= (center - Origin);
        ...
        {
            if (layer != null) this.texture_fullpath = System.IO.Path.Combine(layer.level.root.ContentrootFolder + "\\", texture_filename);

            if (!File.Exists(texture_fullpath))
            {
                DialogResult dr = Forms.MessageBox.Show("The file \"" + texture_fullpath + "\" doesn't exist!\n"
                    + "The texture path is a combination of the root's ContentrootFolder and the TextureItem's relative path.\n"
                    + "Please adjust the XML file before trying to load this root again.\n"
                    + "For now, a dummy texture will be used. Continue loading the root?", "Error loading texture file",
        ...
        public override void OnTransformed()
        {
            transform =
                ...
                Matrix.CreateRotationZ(RotationOrig) *
                Matrix.CreateTranslation(new Vector3(PositionOrig, 0.0f));
        ...
        public override void drawInEditor(SpriteBatch sb)
        {
            ...
            if (this.CustomProperties.ContainsKey("Animated"))
            {
                ...
                sb.Draw(texture, PositionOrig, source, c, RotationOrig, Origin, Scale, se, 0);

            }
            else
            {
                sb.Draw(texture, PositionOrig, null, c, RotationOrig, Origin, Scale, se, 0);
            }
        }
