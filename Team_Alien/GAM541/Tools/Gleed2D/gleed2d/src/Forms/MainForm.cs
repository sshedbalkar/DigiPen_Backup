...
namespace GLEED2D
{
    public partial class MainForm : Form
    {
        ...
        {
            Text = "GLEED2D - " + rootfilename + (DirtyFlag ? "*" : "");
        }
        ...
        {
            if (checkCurrentrootAndSaveEventually() == DialogResult.Cancel) e.Cancel = true;
        }
        ...
        private void treeView1_AfterCheck(object sender, TreeViewEventArgs e)
        {
            if (e.Node.Tag is root)
            {
                Editor.Instance.root.Visible = e.Node.Checked;
            }
            if (e.Node.Tag is Layer)
            {
                Layer l = (Layer)e.Node.Tag;
        ...
        private void treeView1_AfterSelect(object sender, TreeViewEventArgs e)
        {
            ...
            if (e.Node.Tag is Item)
            {
                ...
                Editor.Instance.selectedItem = i;
            }
            if (e.Node.Tag is CustomNode)
            {
                CustomNode c = (CustomNode)e.Node.Tag;
                Editor.Instance.selectCustomNode(c);
                
                //Editor.Instance.SelectedCustomNodes = i;
            }
        }
        ...
        private void treeView1_ItemDrag(object sender, ItemDragEventArgs e)
        {
            ...
            if (((TreeNode)e.Item).Tag is root) return;
        ...
        private void treeView1_DragOver(object sender, DragEventArgs e)
        {
            ...
            if (destnode.Tag is root) return;
        ...
        {
            Editor.Instance.saveroot(filename);
            rootfilename = filename;
            DirtyFlag = false;

            if (Constants.Instance.SaverootStartApplication)
            {
                if (!File.Exists(Constants.Instance.SaverootApplicationToStart))
                {
                    MessageBox.Show("The file \"" + Constants.Instance.SaverootApplicationToStart + "\" doesn't exist!\nPlease provide a valid application executable in Tools -> Settings -> Save root!\nroot was saved.",
                        "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }

                if (Constants.Instance.SaverootAppendrootFilename)
                {
                    Process.Start(Constants.Instance.SaverootApplicationToStart, "\"" + rootfilename + "\"");
                }
                else
                {
                    Process.Start(Constants.Instance.SaverootApplicationToStart);
                }
            }
        ...
        public DialogResult checkCurrentrootAndSaveEventually()
        {
            if (DirtyFlag)
            {
                DialogResult dr = MessageBox.Show("The current root has not been saved. Do you want to save now?", "Save?",
                    MessageBoxButtons.YesNoCancel, MessageBoxIcon.Question);
                if (dr == DialogResult.Yes)
                {
                    if (rootfilename == "untitled")
                    {
                        SaveFileDialog dialog = new SaveFileDialog();
                        dialog.Filter = "XML Files (*.xml)|*.xml";
                        if (dialog.ShowDialog() == DialogResult.OK) saveroot(dialog.FileName);
                        else return DialogResult.Cancel;
                    }
                    else
                    {
                        saveroot(rootfilename);
                    }
                }
        ...
        {
            if (checkCurrentrootAndSaveEventually() == DialogResult.Cancel) return;
            newroot();
        }
        private void FileOpen(object sender, EventArgs e)
        {
            ...
            if (opendialog.ShowDialog() == DialogResult.OK) loadroot(opendialog.FileName);
        }
        private void FileSave(object sender, EventArgs e)
        {
            if (rootfilename == "untitled") FileSaveAs(sender, e);
            else saveroot(rootfilename);
        ...
        private void Runroot(object sender, EventArgs e)
        {
            if (Constants.Instance.RunrootStartApplication)
            {
                if (!System.IO.File.Exists(Constants.Instance.RunrootApplicationToStart))
                {
                    MessageBox.Show("The file \"" + Constants.Instance.RunrootApplicationToStart + "\" doesn't exist!\nPlease provide a valid application executable in Tools -> Settings -> Run root!",
                        "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }

                FileSave(sender, e);
                if (Constants.Instance.RunrootAppendrootFilename)
                {
                    System.Diagnostics.Process.Start(Constants.Instance.RunrootApplicationToStart, "\"" + rootfilename + "\"");
                }
                else
                {
                    System.Diagnostics.Process.Start(Constants.Instance.RunrootApplicationToStart);
                }
            }
        }
        ...
        {
            AddCustomNode f = new AddCustomNode(this);
            f.ShowDialog();
        }

        private void addCustomNodeToolStripMenuItem1_Click(object sender, EventArgs e)
        {
            AddCustomNode f = new AddCustomNode(this);
            f.ShowDialog();
        }
...
namespace GLEED2D1
{
    public class test
    {
        ...
        public test()
        {
            arch_Root = root.FromFile("../../../../xml/gameSettings.xml", GLEED2D.Game1.Instance.Content);

            
        }

        public root arch_Root;
    }
}
