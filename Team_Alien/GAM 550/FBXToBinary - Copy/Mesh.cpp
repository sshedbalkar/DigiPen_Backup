...
void ExtractGeometry(FbxNode* pNode,FbxPose* pPose,MeshVertices& gen,Converter& converter)
{
	...
	if( pNode->GetNodeAttribute() != NULL )
	{
		...
		if( lAttributeType == FbxNodeAttribute::eMesh )
		{
			...
			FbxAMatrix geoTransform = GetGeometry(pNode);
			...
			if( KFbxLayerElementNormal * normLayer = layer->GetNormals() )
			{
				...
				if( normLayer->GetReferenceMode() == KFbxLayerElement::eIndexToDirect )
				...
				else
				{
					...
					if( Mapping == KFbxLayerElement::eByControlPoint )
						gen.NIndices = gen.PositionIndices;

					if( Mapping == KFbxLayerElement::eByPolygonVertex )
			...
			if(  KFbxLayerElementUV * uvLayer = layer->GetUVs() )
			{
				...
				if( uvLayer->GetReferenceMode() == KFbxLayerElement::eIndexToDirect )
...
bool ExtractSkinWeights(FbxPose* pPose,FbxNode* pNode,SkinData& skinData,Skeleton& skel,Converter& converter)
{	
	KString nodeName = pNode->GetName();
		...
		for(int i=0; i<lSkinCount; ++i)
		{
			...
			for (int j=0; j<lClusterCount; ++j)
			{
				...
				if( nodeIndex == -1 )
				{
					...
					pPose->Add(pLink,FbxMatrix(linkBindMatrix));
				}
