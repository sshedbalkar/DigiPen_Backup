...
int main()
{
	...
	result = ReadFileCounts(fn, vertexCount, textureCount, normalCount, faceCount);
	if(!result)
	{
		return -1;
	}
	...
	result = LoadDataStructures(fn, vertexCount, textureCount, normalCount, faceCount);
	if(!result)
	{
		return -1;
	}
...
string GetModelFilename(char* filename)
{
	bool done;
	ifstream fin;
	string s;
	...
	while(!done)
	{
		...
		cout << "Enter model filename (excluding .obj): ";
