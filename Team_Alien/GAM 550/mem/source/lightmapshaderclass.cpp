...
bool LightMapShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	result = InitializeShader(device, hwnd, "Shaders//lightmap.vs", "Shaders//lightmap.ps");
	if(!result)
	{
		return false;
	}
...
bool LightMapShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, char* vsFilename, char* psFilename)
{
	HRESULT result;
	...
	result = D3DX11CompileFromFile(vsFilename, NULL, NULL, "LightMapVertexShader", "vs_4_0", D3D10_SHADER_ENABLE_STRICTNESS, 
	...
	result = D3DX11CompileFromFile(psFilename, NULL, NULL, "LightMapPixelShader", "ps_4_0", D3D10_SHADER_ENABLE_STRICTNESS, 
...
void LightMapShaderClass::OutputShaderErrorMessage(ID3D10Blob* errorMessage, HWND hwnd, char* shaderFilename)
{
	char* compileErrors;
