...
bool HorizontalBlurShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	result = InitializeShader(device, hwnd, "Shaders//horizontalblur.vs", "Shaders//horizontalblur.ps");
	if(!result)
	{
		return false;
	}
...
bool HorizontalBlurShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, char* vsFilename, char* psFilename)
{
	HRESULT result;
...
void HorizontalBlurShaderClass::OutputShaderErrorMessage(ID3D10Blob* errorMessage, HWND hwnd, char* shaderFilename)
{
	char* compileErrors;
