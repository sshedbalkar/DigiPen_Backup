...
bool AlphaMapShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	result = InitializeShader(device, hwnd, "Shaders//alphamap.vs", "Shaders//alphamap.ps");
	if(!result)
	{
		return false;
	}
...
bool AlphaMapShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, char* vsFilename, char* psFilename)
{
	HRESULT result;
	...
	result = D3DX11CompileFromFile(vsFilename, NULL, NULL, "AlphaMapVertexShader", "vs_4_0", D3D10_SHADER_ENABLE_STRICTNESS, 
	...
	result = D3DX11CompileFromFile(psFilename, NULL, NULL, "AlphaMapPixelShader", "ps_4_0", D3D10_SHADER_ENABLE_STRICTNESS, 
...
void AlphaMapShaderClass::OutputShaderErrorMessage(ID3D10Blob* errorMessage, HWND hwnd, char* shaderFilename)
{
	char* compileErrors;
