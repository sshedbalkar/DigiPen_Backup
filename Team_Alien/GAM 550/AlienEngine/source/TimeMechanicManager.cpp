...
namespace Framework
{
	...
	TimeMechanicManager::TimeMechanicManager():m_xSpeed(1)
	{
		m_frameCounter=0;
	...
	void TimeMechanicManager::SetTimeState(TimeState t)
	{
		...
		if(t==TIME_PAUSED && m_timeState!=t)
		{
			AUDIO->pauseAll();
			AUDIO->Pause("main");
			AUDIO->resume("TickTock");
		//	prints("1234");
		}else if(t==TIME_BACK && m_timeState!=t)
		{
			//AUDIO->pauseAll();
			AUDIO->resume("main");
			AUDIO->Pause("TickTock");
			AUDIO->resetFrequency("main");
			AUDIO->reverse("main");
			//prints("asdf");
		}
	...
	void TimeMechanicManager::Update(float dt)
	{
		if(Disabled)
			return;
		//print("TIME_COUNTER" , timecounter);
	...
	void TimeMechanicManager::DecideTimeSpeed()
	{
		if(m_timeState != TIME_BACK) 
		{
			m_xSpeed=1;
		...
		if(timecounter>50 && timecounter<=200)
		{
			m_xSpeed=1;
		}
		...
		else if(timecounter>300 && timecounter<=400)
		{
			m_xSpeed=4;
		}
		else if(timecounter>500 && timecounter<=600)
		{
			m_xSpeed=8;
		}
