...
namespace Framework
{
	...
	void HistoryComponent::updateInitialHistoryData()
	{
		...
		{
			if(this->GetOwner()->has(AutoMovingComponent ) || this->GetOwner()->CompositionTypeId==GOC_Type_Door1 || this->GetOwner()->CompositionTypeId==GOC_Type_Button3|| this->GetOwner()->CompositionTypeId==GOC_Type_Laser||  this->GetOwner()->CompositionTypeId==GOC_Type_Sphere_Thrower ||   this->GetOwner()->CompositionTypeId==GOC_Type_Cube_Thrower )
			{
	...
	void HistoryComponent::Update(HistoryData* hd, bool isBackwards)
	{
		...
		else if((hd->dataType ==  DOMINO_TYPE))
		{
			...
			{
				//HARSHA
				this->GetOwner()->has(Domino)->firstCollide = false;
