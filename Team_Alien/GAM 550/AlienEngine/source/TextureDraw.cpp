...
namespace Framework
{
	...
	int TextureData::Init()
	{
		...
		if( !m_bitmap->Initialize(GRAPHICS->m_D3D->GetDevice(), (float)GRAPHICS->screen_width, (float)GRAPHICS->screen_height, m_fileName, 0, 0) )
		{
	...
	//
	int TextureDraw::SetDimensions( unsigned int id, const unsigned width, const unsigned height )
	{
		TextureListType::iterator it1 = l_textures.find( id );
		if( it1 != l_textures.end() )
		{
			it1->second->m_width = width;
			it1->second->m_height = height;
			it1->second->m_bitmap->SetDimension( (float)width, (float)height );
			//
			return RET_SUCCESS;
		}
		//
