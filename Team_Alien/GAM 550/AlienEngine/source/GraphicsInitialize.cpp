...
FxaaShader(NULL),
FxaaRT(NULL), 
maxObjectCount_thisLevel(0), 
pCubeModelThisFrameArray(NULL), 
m_pInstancing(NULL)
{
	GRAPHICS = this;
...
void Graphics::ChangeResolution(int w, int h)
{
	...
	// Initialize the bitmap object.
	result = m_Credits->Initialize(m_D3D->GetDevice(), float(screen_width), float(screen_height), "Assets//textures//Credits.png", screen_width, screen_height);
	if(!result) __debugbreak();

	// Initialize the bitmap object.
	result = m_1x->Initialize(m_D3D->GetDevice(), float(screen_width), float(screen_height), "Assets//textures//1.png", 150, 100);
	if(!result) __debugbreak();

	// Initialize the bitmap object.
	result = m_2x->Initialize(m_D3D->GetDevice(), float(screen_width), float(screen_height), "Assets//textures//2.png", 150, 100);
	if(!result) __debugbreak();

	// Initialize the bitmap object.
	result = m_4x->Initialize(m_D3D->GetDevice(), float(screen_width), float(screen_height), "Assets//textures//4.png", 150, 100);
	if(!result) __debugbreak();

	// Initialize the bitmap object.
	result = m_8x->Initialize(m_D3D->GetDevice(), float(screen_width), float(screen_height), "Assets//textures//8.png", 150, 100);
	if(!result) __debugbreak();

	// Initialize the bitmap object.
	result = m_16x->Initialize(m_D3D->GetDevice(), float(screen_width), float(screen_height), "Assets//textures//16.png", 150, 100);
	if(!result) __debugbreak();
...
void Graphics::Initialize()
{
	...
	result = m_pInstancing->InitializeInstancing( m_D3D->GetDevice() );
	if(!result)
	{
		MessageBox(hwnd, "Could not initialize the Instancing class.", "Error", MB_OK);
		return;
	}
