...
namespace Framework
{
	...
	void GameLogic::Initialize()
	{
		...
		else
		{
			...
			const bool DataDrivenObjects = false;
			...
			{
				HERO = FACTORY->CreateEmptyComposition();
				Transform * transform = new Transform();
				transform->Position = Vec3( 0.0f,0.0f,3.0f );
				GRAPHICS->RegisterTriangle( transform );

				HERO->AddComponent(CT_Transform , transform );
				Body * body = new Body();
				body->Mass = 3.0f;
				body->Restitution = 0.3f;
				body->Friction = 0.3f;
				ShapeAAB * box = new ShapeAAB();
				box->Extents = Vec2(1.1,1.1);
				body->BodyShape = box;	
				HERO->AddComponent(CT_Body, body );

				GOC* temp = FACTORY->CreateEmptyComposition();
				Transform * ransform = new Transform();
				ransform->Position = Vec3( 3.0f,3.0f,3.0f );
				GRAPHICS->RegisterTriangle( ransform );

				temp->AddComponent(CT_Transform , ransform );
				Body *ody = new Body();
				ody->SetPosition(Vec2(3,3));
				ody->Mass = 3.0f;
				ody->Restitution = 0.3f;
				ody->Friction = 0.3f;
				ShapeAAB *ox = new ShapeAAB();
				ox->Extents = Vec2(1.1,1.1);
				ody->BodyShape =  ox;		
				temp->AddComponent(CT_Body, ody );
		...
		{			
			if( (int)timeGetTime() - SpawnTime > Fuse )
			{
				...
				{			
					...
					for(int i=-1;i<=1;++i)
					{
						Vec2 dir( sin( float(i)*(float)D3DX_PI*0.3f) , cos( float(i)*(float)D3DX_PI*0.3f) );		
