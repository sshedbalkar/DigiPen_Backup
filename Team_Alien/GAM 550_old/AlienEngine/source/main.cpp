...
#endif
{
	...
	WindowsSystem* windows = new WindowsSystem(windowTitle, ClientWidth, ClientHeight, false );
...
//	wc.hIcon= LoadIcon(windows->hInstance , "Assets/textures/Icon.ico");
	...
	engine->AddSystem( new Resources() );
	...
	engine->AddSystem( new ScriptEngine() );
	//TODO: Add additional systems, such as audio, and possibly xinput, lua, etc.
	engine->Initialize();

	ID3D11Device* device = graphics->GetDevice();
	editor->InitializeTweakBar( device, ClientWidth, ClientHeight );
