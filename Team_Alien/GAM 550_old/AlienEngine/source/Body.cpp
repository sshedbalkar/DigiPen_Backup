	...
	void Body::Serialize(ISerializer& stream)
	{
		...
		if(str->isGood())
		{
			...
			else if(!s.compare("Box"))
			{
				...
				str->goToParent();
				str->readProperty("DimensionHalf", *str);
				str->readValue("value", shape->Extents, 2);
				this->BodyShape = shape;
	...
	void Body::SendMessage(Message *m)
	{
		switch( m->MessageId )
		{
			case Mid::Collide:
			{
				...
				if(msg->ContactNormal.y==1)
					isCollidingDown=true;
				if(msg->ContactNormal.y==-1)
					isCollidingUp=true;
				if(msg->ContactNormal.x==1)
					isCollidingLeft=true;
				if(msg->ContactNormal.x==-1)
					isCollidingRight=true;

				
				//if(msg->CollideObject->CompositionTypeId==GOC_Type_Hero && isCollidingDown)
				//	msg->CollideObject->has(HeroProperties)->collidingDownWith=msg->CollidedWith;
				//else if(msg->CollideObject->CompositionTypeId==GOC_Type_Hero && !isCollidingDown)
				//	msg->CollideObject->has(HeroProperties)->collidingDownWith=NULL;
