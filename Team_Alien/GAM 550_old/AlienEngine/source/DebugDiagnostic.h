...
{
#ifdef _DEBUG
#define DXCall(call)	{ hr = (call); if ( FAILED(hr) ) { std::string errmsg; errmsg = DXGetErrorDescription( hr ); printf( "DX Error: %s(%d)%s\n", __FILE__, (DWORD)__LINE__, errmsg.c_str() ); } }
#else
#define DXCall(call) (call);
#endif
