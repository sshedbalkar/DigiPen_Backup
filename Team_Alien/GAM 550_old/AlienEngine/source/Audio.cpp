...
//##		Play("background");
	...
	Sound* Audio::Load(std::string filename)
	{
	...
	bool Audio::Load(std::string filename,std::string name,bool loop,float volume)
	{
	...
	bool Audio::soundExists(std::string name)
	{
		for(Iterator it= sounds.begin();it != sounds.end(); ++it)
		{
	...
	Sound* Audio::findSound(std::string name)
	{
		Sound *sound=NULL;
		for(Iterator it= sounds.begin();it != sounds.end(); ++it)
		{
	...
	bool Audio::Play(std::string name,int loopCount)
	{
		if(isPlaying(name)) 
			return true;
		FMOD_RESULT result;
...
//##		FMOD_ChannelGroup_SetVolume(maingrp,(float)std::max(0,std::min(volume,1)));
	}

	void Audio::changeVolume(std::string name,float volume)
	{
		if(!isPlaying(name)) return;
		Sound *sound=findSound(name);
		if(sound==NULL) return;
	...
	void Audio::resume(std::string name)
	{
