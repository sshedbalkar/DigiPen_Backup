...
void RigidBody::Initialize()
{
	...
	orientation = Quaternion(transform->rotation.w,transform->rotation.x,transform->rotation.y,transform->rotation.z);
...
void RigidBody::Serialize(ISerializer& stream)
{
	...
	if(str->isGood())
	{
		...
		str->readValue("acceleration",tempVec,3);
			acceleration=Vector3(tempVec.x,tempVec.y,tempVec.z);
...
void RigidBody::PublishResults()
{
	...
	transform->rotation=D3DXQUATERNION(orientation.i,orientation.j,orientation.k,orientation.r);
}
