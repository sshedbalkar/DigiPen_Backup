...
void CollisionBox::Serialize(ISerializer& stream)
{
	...
	{
		Vec3 temp;
		str->readValue("HalfSize", temp, 3);
		halfSize=Vector3(temp.x,temp.y,temp.z);
	}
	delete str;
}
...
{
	Vector3 centre = sphere.getAxis(3);
	Vector3 rayToCentre = centre-ray.position;

	real v = rayToCentre*ray.direction;
	real distance = (sphere.radius*sphere.radius) - ((rayToCentre*rayToCentre)-(v*v));

	if(distance<0)
		return 0;
	else
	{
		real d=sqrt(distance);
		pointOfIntersection = ray.position + ray.direction*(v-d);
		return 1;
	}
...
unsigned CollisionDetector::rayCastBox(const CollisionBox &box,const Ray &ray,Vector3 &pointOfIntersection)
{
	...
	Vector3 point = rayPos + (rayDir*t_min);
	for(int i=0;i<3;++i)
	{
		if(point[i]<0.01) point[i]=0;
	}
