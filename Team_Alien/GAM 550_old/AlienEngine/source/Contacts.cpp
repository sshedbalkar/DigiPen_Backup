...
void Contact::matchAwakeState()
{
    ...
    if (!body[1] || !(body[1]->hasFiniteMass()) || !(body[0]->hasFiniteMass())) 
		return;
...
{
    const static real velocityLimit = (real)0.15f;//or 0.25f
...
inline Vector3 Contact::calculateFrictionlessImpulse(Matrix3 * inverseInertiaTensor)
{
	...
	//no friction so only impulse in the direction of normal is enuf
    ...
    for (unsigned i = 0; i < 2; i++) 
	{
		if (body[i])
		{
			...
			else
			{
				...
				Vector3 targetAngularDirection = relativeContactPosition[i].crossProduct(contactNormal);
        ...
        for (unsigned i = 0; i < numContacts; i++)
        {
            for (unsigned b = 0; b < 2; b++) if (c[i].body[b])
            {
                for (unsigned d = 0; d < 2; d++)
                {
                    if (c[i].body[b] == c[index].body[d])
                    {
                        deltaVel = velocityChange[d] + rotationChange[d].crossProduct(c[i].relativeContactPosition[b]);
